...
const STEP_DURATION = 500; // ms to move one card
const STEP_PAUSE = 1500; // ms to wait between steps

export default function MarketsSlider({ refreshTrigger }) {
	...
	const fetchMarkets = async () => {
		try {
			...
			// Guest (or no city) => all markets.
			const url = city
				? `https://frischly-dash-leb.onrender.com/api/markets/public?city=${encodeURIComponent(
						city
				  )}`
				: "https://frischly-dash-leb.onrender.com/api/markets/public";
	...
	useEffect(() => {
		if (refreshTrigger > 0) fetchMarkets();
	}, [refreshTrigger]);
	...
	const renderMarket = (market, index) => (
		<TouchableOpacity
			...
			onPress={() =>
				router.push(
					`/shop?market=${market._id}&marketName=${encodeURIComponent(
						market.name || ""
					)}`
				)
			}
